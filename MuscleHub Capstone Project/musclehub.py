...
plt.title("Percent of users in each group")
...
member_pivot = member_group.pivot(index="ab_test_group", columns="is_member", values="first_name").reset_index()
...
#from scipy.stats import chi2_contingency
chi2_member, pval_member, dof_member, expected_member = chi2_contingency([[200, 50], [250, 75]])
pval_member
...
df_group = df.groupby(["ab_test_group", "is_member"]).first_name.count().reset_index()
final_member_pivot = df_group.pivot(index="ab_test_group", columns="is_member", values="first_name").reset_index()
...
#from scipy.stats import chi2_contingency
chi2_df, pval_df, dof_df, expected_df = chi2_contingency([[200, 2304], [250, 2250]])
pval_df
...
ax1.set_xticks(range(len(app_pivot["Percent with Application"])))
...
ax3.set_xticks(range(len(final_member_pivot["Percent Purchase"])))
ax3.set_xticklabels(["Fitness Test", "No Fitness Test"])
ax3.set_yticks([0, 0.05, 0.10])
ax3.set_yticklabels(["0%", "5%", "10%"])
